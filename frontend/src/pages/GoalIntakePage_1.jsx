    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
        <div className="flex justify-between items-start">
            <div className="flex-1">
                ...
                <div className="text-xs text-slate-500 mt-1">Confirmed 路 Click edit to modify</div>
            </div>
            <div className="flex items-center gap-2">
                {onEdit && (
                    <button
...
const Copilot = ({ 
    ...
    const handlePermissionResponse = async (granted) => {
        ...
        if (!granted) {
            ...
            setMessages(prev => [...prev, { 
                role: 'system', 
                text: '馃敀 Access denied. AI will provide generic advice without your financial data.' 
            }]);
    ...
    const handleSend = async (overrideText) => {
        ...
        if (needsPermission && mode !== 'ask') {
            ...
            setRequestedDataTypes(requiredTypes); // 瀛樺偍闇€瑕佺殑鏁版嵁绫诲瀷
            setSelectedAllowlist(requiredTypes); // 榛樿鍏ㄩ€夋墍闇€鏁版嵁
...
const GoalEnginePage = () => {
  ...
  const handleContextUpdate = useCallback((updates) => {
      ...
      // 鉁?Also extract from nested goal_details if present
      ...
      // 鉁?Track which root-level fields are being updated (before Object.assign)
      ...
      setGoalContext(prev => {
          ...
          if (detailsFound) {
              ...
              // 鉁?Clear stale AI values when user modifies goal_details
          ...
          // 鉁?Also clear AI values when user modifies root-level fields (like target_amount)
      ...
      try {
          ...
          setMessages(prev => {
              ...
              if (updated[aiMsgIndex]) {
                  updated[aiMsgIndex] = {
                      ...updated[aiMsgIndex],
                      text: `鈿狅笍 Error executing ${nextSubId}. Please try again.`,
                      isTyping: false,
                      isStreaming: false
                  };
              }
  ...
  const handleSubstageSubmit = async (stageId, subId, payload) => {
    ...
    if (stageId === 'definition') {
        if (!nextSubId) {
            ...
            setMessages(prev => [...prev, { 
                role: 'system', 
                text: "鉁?All substages completed! Click 'Confirm & continue' below to proceed to Strategy stage." 
            }]);
            return;
        }
        ...
        if (needsPermission) {
            ...
            setMessages(prev => [...prev, { 
                role: 'user', 
                text: `鉁?${confirmedLabel} confirmed` 
            }]);
  ...
  const runStageAnalysis = async (stageId, context, stageIdx) => {
      ...
      try {
          ...
          // === 鍓嶇涓诲锛氳绠楁渶鏂拌储鍔″揩鐓?===
          // currentGoalId 宸插湪 line 1615 澹版槑锛岀洿鎺ヤ娇鐢?
          ...
          // 璁＄畻 monthly_surplus_allocatable锛堟姇璧勫墠鐩堜綑锛屾墸闄?reserve 鍜?other goals 鍚庣殑鍙垎閰嶇泩浣欙級
          ...
          const data = await goalEngineService.generateDecisionStream({
              ...
              useRag,
              allowAIDataSharing // 馃敀 Pass chatbox privacy toggle
          }, (chunk) => {
              accumulatedRaw += chunk;
              const streamingThought = extractField('thought_process', accumulatedRaw);
              const streamingRationale = extractField('rationale', accumulatedRaw);
  ...
  return (
      ...
      <div className="max-w-[2400px] mx-auto p-1.5 md:p-3 lg:p-4 h-[calc(100vh-64px)] flex flex-col animate-fade-in overflow-hidden">
        ...
        >
            ...
            >
                {collapsed !== 'right' && (
                <>
                {/* 馃帗 Welcome Guide - Shows on first visit or after clear */}
   ...
   {currentStage === 0 && (
    ...
    return order.map((subId, idx) => {
      ...
      {/* 馃帗 Help Modal - Accessible anytime via ? button */}
      {showOnboardingModal && (
        <div 
