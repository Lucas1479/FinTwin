...
import { X, Upload, Check, ScanLine, DollarSign, Calendar, Building, Tag, Percent, ArrowRight } from 'lucide-react';
import { createAsset } from '../../services/wealthService';
...
const AddAssetModal = ({ isOpen, onClose, onRefresh }) => {
  const [recordType, setRecordType] = useState('Asset'); // 'Asset' | 'Liability'
  ...
  const handleFileUpload = (e) => {
    ...
    setTimeout(() => {
      ...
      // Mock extracted data based on random logic or filename
            ...
            {selectedCategory === 'Cash_TermDeposit' && (
              <>
                ...
                <InputField label="Maturity Date" type="date" 
                  value={formData.details.maturity_date} onChange={v => updateDetail('maturity_date', v)} />
              </>
            )}
          </>
        );
        ...
        return (
          <>
            ...
            <InputField label="Fixed Until" type="date" 
              value={formData.details.fixed_until} onChange={v => updateDetail('fixed_until', v)} />
          </>
        );
      default:
        return (
        ...
        <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10">
          <div>
            <h2 className="text-xl font-bold text-slate-900">
              {step === 1 ? 'Add New Item' : `Add ${CATEGORY_LABELS[selectedCategory]}`}
            </h2>
            <p className="text-sm text-slate-500 mt-0.5">
              {step === 1 ? 'Select the type of asset or liability to add' : 'Fill in the details below'}
            </p>
          </div>
