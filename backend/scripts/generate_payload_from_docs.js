...
const CONFIG = {
  ...
  OUTPUT_FILE: path.join(__dirname, '../fintwin_vectara_payload.json'),
  CHUNK_SIZE: 1000, // 字符数per chunk
};
...
const extractTextFromDOCX = async (filePath) => {
  try {
    ...
    return {
      text: result.value,
      pages: Math.ceil(result.value.length / 3000), // 估算页数
      metadata: {}
    };
  } catch (err) {
...
// 智能分块（保留上下文）
// ============================================
const chunkTextWithContext = (text, maxSize = CONFIG.CHUNK_SIZE) => {
...
const processDocument = async (fileName) => {
  ...
  if (!extracted.text || extracted.text.length < 100) {
    console.log(`   ⚠️ No meaningful content extracted (${extracted.text.length} chars) - possibly scanned/image PDF`);
    return null;
  }
  
  // Check if content is mostly page numbers (scanned PDF indicator)
  const pageNumberPattern = /^--\s*\d+\s+of\s+\d+\s*--$/;
  const lines = extracted.text.split('\n').filter(l => l.trim());
  const pageNumberLines = lines.filter(l => pageNumberPattern.test(l.trim()));
  const pageNumberRatio = pageNumberLines.length / Math.max(lines.length, 1);
  
  if (pageNumberRatio > 0.5) {
    console.log(`   ⚠️ Scanned/image PDF detected (${pageNumberRatio.toFixed(1)}% page numbers) - ${extracted.text.length} chars`);
    console.log(`   💡 Suggestion: Use OCR tool or find text-based version of this PDF`);
    return null;
  }
  ...
  // 智能分块
  const chunks = chunkTextWithContext(extracted.text);
...
const generatePayload = async () => {
  console.log('🚀 Generate Payload from Docs');
  console.log('================================\n');
  ...
  if (supportedFiles.length === 0) {
    console.error('\n❌ No supported documents found in docs directory');
    console.error('   Supported formats: .pdf, .docx, .doc');
    process.exit(1);
  }
  ...
  console.log('\n💡 Next step: Run npm run smart-process with improved prompt to filter out TOC/headers');
};
